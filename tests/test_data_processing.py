...
class TestPad2DTensor:
    ...
    def test_pads_to_maximum_dimensions(self, sample_tensors):
        ...
        assert torch.allclose(result[0], torch.tensor([[1, 2, 0], [3, 4, 0], [0, 0, 0]], dtype=torch.long))
...
def mock_config():
    ...
    config.represent_spans = False
    config.neg_spans_ratio = 1.0
    config.subtoken_pooling = "first"
    config.fine_tune = True
    config.words_splitter_type = "whitespace"
    return config
...
class TestBiEncoderSpanProcessor:
    ...
    def processor(self, mock_config, mock_tokenizer, mock_words_splitter):
        ...
        return BiEncoderSpanProcessor(mock_config, mock_tokenizer, mock_words_splitter, labels_tokenizer)
    ...
    def test_initialization_with_labels_tokenizer(self, mock_config, mock_tokenizer, mock_words_splitter):
        ...
        processor = BiEncoderSpanProcessor(mock_config, mock_tokenizer, mock_words_splitter, labels_tokenizer)
...
class TestUniEncoderSpanDecoderProcessor:
    ...
    def processor(self, mock_config, mock_tokenizer, mock_words_splitter):
        ...
        return UniEncoderSpanDecoderProcessor(mock_config, mock_tokenizer, mock_words_splitter, decoder_tokenizer)
    ...
    def test_initialization_with_decoder_tokenizer(self, mock_config, mock_tokenizer, mock_words_splitter):
        ...
        processor = UniEncoderSpanDecoderProcessor(mock_config, mock_tokenizer, mock_words_splitter, decoder_tokenizer)
...
class TestRelationExtractionSpanProcessor:
    ...
    def test_create_relation_labels_adjacency_matrix(self, processor):
        ...
        batch = {
            "tokens": [["John", "works", "at", "Google"]],
            "span_label": torch.tensor([[1, 0, 0, 2, 0, 0]]),
            "span_mask": torch.tensor([[True, False, False, True, False, False]]),
