...
class TestExtractPromptFeatures:
    ...
    def test_extracts_class_token_embeddings(self, prompt_setup):
        ...
        assert torch.allclose(prompts_embedding[0, 0], prompt_setup["token_embeds"][0, 2])
        assert torch.allclose(prompts_embedding[0, 1], prompt_setup["token_embeds"][0, 5])
        assert torch.allclose(prompts_embedding[0, 2], prompt_setup["token_embeds"][0, 8])
    ...
    def test_extracts_next_token_when_embed_ent_token_false(self, prompt_setup):
        ...
        prompts_embedding, prompts_mask = extract_prompt_features(**prompt_setup, embed_ent_token=False)

        # Should extract from positions 3, 6, 9 (one after class tokens)
        assert torch.allclose(prompts_embedding[0, 0], prompt_setup["token_embeds"][0, 3])
        assert torch.allclose(prompts_embedding[0, 1], prompt_setup["token_embeds"][0, 6])
        assert torch.allclose(prompts_embedding[0, 2], prompt_setup["token_embeds"][0, 9])
    ...
    def test_pads_with_zeros(self, prompt_setup):
        ...
        assert torch.allclose(prompts_embedding[1, 2], torch.zeros(prompt_setup["embed_dim"]))
...
class TestExtractPromptFeaturesAndWordEmbeddings:
    ...
    def test_prompt_embeddings_correct(self, combined_setup):
        ...
        assert torch.allclose(prompts_embedding[0, 0], combined_setup["token_embeds"][0, 1])
        assert torch.allclose(prompts_embedding[1, 0], combined_setup["token_embeds"][1, 2])
    ...
    def test_word_embeddings_correct(self, combined_setup):
        ...
        assert torch.allclose(words_embedding[0, 0], combined_setup["token_embeds"][0, 3])
        assert torch.allclose(words_embedding[0, 1], combined_setup["token_embeds"][0, 7])
...
class TestBuildEntityPairs:
    ...
    def basic_pairs_setup(self):
        ...
        return {"adj": adj, "span_rep": span_rep, "threshold": 0.5, "B": B, "E": E, "D": D}
    ...
    def test_handles_no_pairs_above_threshold(self):
        ...
        pair_idx, pair_mask, head_rep, tail_rep = build_entity_pairs(adj, span_rep, threshold=0.5)
    ...
    def test_different_thresholds(self, threshold):
        ...
        pair_idx, pair_mask, head_rep, tail_rep = build_entity_pairs(adj, span_rep, threshold=threshold)
...
class TestBaseModel:
    ...
    def test_loss_basic_computation(self, mock_config):
        ...
        losses = model._loss(logits, labels, alpha=-1.0, gamma=0.0)
...
class TestUniEncoderSpanModel:
    ...
    def test_loss_computation(self, mock_config):
        ...
        loss = model.loss(scores, labels, prompts_embedding_mask, span_mask, alpha=-1.0, gamma=0.0, reduction="sum")
    ...
    def test_loss_with_masking(self, mock_config):
        ...
        loss = model.loss(scores, labels, prompts_embedding_mask, span_mask, reduction="sum")
...
class TestUniEncoderTokenModel:
    ...
    def test_loss_computation(self, mock_config):
        ...
        loss = model.loss(scores, labels, prompts_embedding_mask, mask, alpha=-1.0, gamma=0.0, reduction="sum")
    ...
    def test_loss_with_word_masking(self, mock_config):
        ...
        loss = model.loss(scores, labels, prompts_embedding_mask, mask, reduction="sum")
...
class TestUniEncoderSpanDecoderModel:
    ...
    def test_select_decoder_embedding_shape(self, mock_config):
        ...
        target_rep, target_mask, sel_idx = model.select_decoder_embedding(representations, rep_mask)
    ...
    def test_get_raw_decoder_inputs(self, mock_config):
        ...
        span_tokens, span_tokens_mask = model.get_raw_decoder_inputs(representations, rep_mask)
...
class TestBiEncoderSpanModel:
    ...
    def test_forward_with_precomputed_labels_embeds(self, mock_config, model_inputs):
        ...
        model_inputs_with_embeds = {
            k: v for k, v in model_inputs.items() if k not in ["labels_input_ids", "labels_attention_mask", "labels"]
        }
    ...
    def test_loss_reduction_mean(self, mock_config):
        ...
        loss = model.loss(scores, labels, prompts_embedding_mask, span_mask, reduction="mean")
