...
def calculate_span_score(start_idx, end_idx, scores_inside_i, start_i, end_i, id_to_classes):
    span_i = []
...
def predict():
        ...
        for batch in tqdm(data_loader):
            ...
            try:
                ...
                for batch_idx, _ in enumerate(batch["tokens"]):
                    raw_gold_spans = batch["entities"][batch_idx]
                    if len(raw_gold_spans) == 0:
                        print()
                    ...
                    annotations.append({**original_input[0], "gold_spans_with_confidence": gold_spans})
                
            except Exception as e:
                errors.append(original_input[0])
    ...
    with open(f"/vol/tmp/goldejon/multilingual_ner/data/confidence_annotations/errors.json", "w") as f:
        json.dump(errors, f)
